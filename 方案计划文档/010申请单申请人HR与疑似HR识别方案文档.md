...
- `组织发展` -> `组织发展`
- `人力业务支持` -> `HRBP/业务支持`
- `薪酬绩效` -> `薪酬绩效岗`（fallback 靠后）
- `人事运营` -> `人事运营`（fallback 靠后）
...
- `组织路径名称` 以 `万物云_万物云本部_人力资源与行政服务中心_BG人力资源行政服务中心` 开头时，优先按分组映射：
  ...
  - 包含 `平台与运营组` -> `人事运营`
  - 包含 `人才与领导力组` -> `人才发展（含培训认证）`
  - 包含 `组织与效能组` 且 `标准岗位名称 = 薪酬绩效岗` -> `薪酬绩效岗`
  - 包含 `组织与效能组` 且不满足上条 -> `组织发展岗`
  - 包含 `员工体验与行政组` -> `人事行政岗`
- `标准岗位名称 = HRBP` 且 `职位名称 = 绩效专家` -> `薪酬绩效岗`
- `职位名称` 同时包含 `项目` 与 `负责人` -> `人力资源岗`（不再落管理岗）
- `职位名称` 包含 `专业总监` 时按专项关键词覆盖：
  - 含 `数字化运营` / `合规运营` -> `人事运营`
  - 含 `人力资源专业总监` -> `HRBP/业务支持`
  - 含 `组织效率与员工体验` / `组织机制与人才发展` -> `组织发展岗`
- `职位名称` 同时包含 `组织` 与 (`薪酬` 或 `绩效`) 时：
  - 若 `二级职能名称 = 薪酬绩效` -> `薪酬绩效岗`
  - 否则 -> `组织发展岗`
- `二级职能名称` / `职位名称` / `标准岗位名称` 包含 `薪酬绩效` 或 `薪酬福利` -> `薪酬绩效岗`
- `二级职能名称 = 薪酬绩效` -> `薪酬绩效岗`
- `职位名称` / `标准岗位名称` 包含 `薪酬` 或 `绩效` -> `薪酬绩效岗`
- 服务站细分：`属地服务站` 场景下，`二级职能名称` 包含 `招聘` -> `服务站-招聘`，否则 `服务站-人事运营`
- 标签更名统一：
  - `HRBP` -> `HRBP/业务支持`
  - `招聘岗` / `招聘` -> `招聘配置岗`
  - `组织发展` -> `组织发展岗`
  - `人力行政岗` -> `人事行政岗`
  - `培训认证与人才发展` -> `人才发展（含培训认证）`
...
- `hr_judgement_reason`
  - 便于审计追溯，例如：
    ...
    - `weak_signal_position_demoted_to_hx`
