...
APPLICANT_HR_SUBDOMAIN_HRBP_KEYWORD_PATTERN = re.compile(r"HRBP", re.IGNORECASE)
    ...
    def _build_hr_subdomain(
        ...
        *,
        ...
        if (
            ...
            and (
                position_name in APPLICANT_HR_SUBDOMAIN_TRAINING_CERT_POSITION_WHITELIST
                or (
                    position_name is not None
                    and APPLICANT_HR_SUBDOMAIN_TRAINING_CERT_POSITION_KEYWORD_PATTERN.search(position_name)
                )
            )
        ):
            return APPLICANT_HR_SUBDOMAIN_TRAINING_CERT_TALENT
        ...
        if (
            position_name is not None
            and (
                APPLICANT_HR_H2_MANAGEMENT_POSITION_PATTERN.search(position_name)
                or APPLICANT_HR_SUBDOMAIN_MANAGEMENT_POSITION_KEYWORD_PATTERN.search(position_name)
            )
        ) or (
        ...
        if any(
            ...
            return "人事运营"
        if any(
            field is not None and APPLICANT_HR_SUBDOMAIN_PERSONNEL_KEYWORD in field
            for field in (position_name, standard_position_name)
        ):
            return "人力资源岗"
                ...
                roster_profiles[employee_no].update(
                    {
                        ...
                        "employee_group": self._strip_text(row[5]),
                        "employee_subgroup": self._strip_text(row[6]),
                        "level1_function_name": self._strip_text(row[7]),
                        "level2_function_name": self._strip_text(row[8]),
                        "position_name": self._strip_text(row[9]),
                        "standard_position_name": self._strip_text(row[10]),
    ...
    def _fetch_person_attributes_map(self, cursor, employee_nos: Iterable[str]) -> dict[str, dict[str, Any]]:
        ...
        cursor.execute(
            ...
            SELECT
                ...
                {self._quote_identifier(PERSON_ATTRIBUTES_COLUMNS["org_unit_name"])},
        ...
        return {
            self._strip_text(row[0]) or "": {
                ...
                "level1_function_name": self._strip_text(row[4]),
                "position_name": self._strip_text(row[5]),
                "org_path_name": self._strip_text(row[6]),
                "hr_type": self._strip_text(row[7]),
                "roster_match_status": self._strip_text(row[8]),
                "hr_judgement_reason": self._strip_text(row[9]),
            }
            for row in rows
            if self._strip_text(row[0]) is not None
        }
...
class PostgresChatStore(_PostgresStoreBase):
    ...
    def _schema_key(cls, settings: DatabaseSettings) -> tuple[str, int, str, str]:
        return (settings.host, settings.port, settings.dbname, settings.schema)

    def ensure_table(self) -> None:
        schema_key = self._schema_key(self.settings)
        if schema_key in self._ensured_schema_keys:
            return
    ...
    @classmethod
    def _session_row_to_payload(cls, row: tuple[Any, ...]) -> dict[str, Any]:
        return {
            "sessionId": str(row[0]),
            "title": str(row[1] or ""),
            "workspaceDir": str(row[2] or ""),
            "modelProvider": str(row[3] or ""),
            "modelName": str(row[4] or ""),
            "status": str(row[5] or ""),
            "createdAt": cls._format_temporal_value(row[6]),
            "lastActiveAt": cls._format_temporal_value(row[7]),
        }

    @classmethod
    def _message_row_to_payload(cls, row: tuple[Any, ...]) -> dict[str, Any]:
        token_count = row[4]
        return {
            "messageId": str(row[0]),
            "sessionId": str(row[1]),
            ...
            "tokenCount": int(token_count) if token_count is not None else None,
            "createdAt": cls._format_temporal_value(row[5]),
        }
    ...
    def create_session(
        ...
        *,
        ...
        self.ensure_table()
        insert_columns = [
            CHAT_SESSION_COLUMNS["session_id"],
            CHAT_SESSION_COLUMNS["title"],
            CHAT_SESSION_COLUMNS["workspace_dir"],
            CHAT_SESSION_COLUMNS["model_provider"],
            CHAT_SESSION_COLUMNS["model_name"],
            CHAT_SESSION_COLUMNS["status"],
            CHAT_SESSION_COLUMNS["created_at"],
            CHAT_SESSION_COLUMNS["last_active_at"],
        ]
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    ORDER BY {self._quote_identifier(CHAT_SESSION_COLUMNS["last_active_at"])} DESC
                    LIMIT %s OFFSET %s
                    """,
                    (max(limit, 1), max(offset, 0)),
                )
                rows = cursor.fetchall()
        return [self._session_row_to_payload(row) for row in rows]
    ...
    def get_session(self, session_id: str) -> dict[str, Any] | None:
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    FROM {CHAT_SESSION_TABLE}
                    WHERE {self._quote_identifier(CHAT_SESSION_COLUMNS["session_id"])} = %s
                    LIMIT 1
                    """,
                    (session_id,),
                )
        ...
        if row is None:
            return None
        return self._session_row_to_payload(row)
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    SET
                        {self._quote_identifier(CHAT_SESSION_COLUMNS["status"])} = %s,
                        {self._quote_identifier(CHAT_SESSION_COLUMNS["last_active_at"])} = NOW()
                    WHERE {self._quote_identifier(CHAT_SESSION_COLUMNS["session_id"])} = %s
                    """,
                    (status, session_id),
                )
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    f"""
                    UPDATE {CHAT_SESSION_TABLE}
                    SET {self._quote_identifier(CHAT_SESSION_COLUMNS["last_active_at"])} = NOW()
                    WHERE {self._quote_identifier(CHAT_SESSION_COLUMNS["session_id"])} = %s
                    """,
                    (session_id,),
                )
    ...
    def create_message(
        ...
        *,
        ...
        self.ensure_table()
        insert_columns = [
            CHAT_MESSAGE_COLUMNS["message_id"],
            CHAT_MESSAGE_COLUMNS["session_id"],
            CHAT_MESSAGE_COLUMNS["role"],
            CHAT_MESSAGE_COLUMNS["content"],
            CHAT_MESSAGE_COLUMNS["token_count"],
            CHAT_MESSAGE_COLUMNS["created_at"],
        ]
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
    ...
    def get_message(self, message_id: str) -> dict[str, Any] | None:
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    WHERE {self._quote_identifier(CHAT_MESSAGE_COLUMNS["message_id"])} = %s
                    LIMIT 1
                    """,
                    (message_id,),
                )
        ...
        if row is None:
            return None
        return self._message_row_to_payload(row)
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    SET
                        {self._quote_identifier(CHAT_MESSAGE_COLUMNS["content"])} = %s,
                        {self._quote_identifier(CHAT_MESSAGE_COLUMNS["token_count"])} = %s
                    WHERE {self._quote_identifier(CHAT_MESSAGE_COLUMNS["message_id"])} = %s
                    """,
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    FROM {CHAT_MESSAGE_TABLE}
                    WHERE {self._quote_identifier(CHAT_MESSAGE_COLUMNS["session_id"])} = %s
                    ORDER BY {self._quote_identifier(CHAT_MESSAGE_COLUMNS["created_at"])} ASC
                    LIMIT %s
                    """,
                    (session_id, max(limit, 1)),
                )
                rows = cursor.fetchall()
        return [self._message_row_to_payload(row) for row in rows]
    ...
    def append_execution_log(
        ...
        *,
        ...
        self.ensure_table()
        insert_columns = [
            CHAT_EXECUTION_LOG_COLUMNS["log_id"],
            CHAT_EXECUTION_LOG_COLUMNS["session_id"],
            CHAT_EXECUTION_LOG_COLUMNS["message_id"],
        ...
        with self.connect() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    f"""
                    INSERT INTO {CHAT_EXECUTION_LOG_TABLE} (
                        {self._quoted_columns(insert_columns)}
                    ) VALUES (
                        %s, %s, %s, %s, %s, %s, NOW()
                    )
