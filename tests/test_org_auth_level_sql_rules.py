...
class OrgAuthLevelSqlRulesTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.migration_sql = (self.repo_root / "automation/sql/027_wanrui_org_unit_auth_level_exception.sql").read_text(
            encoding="utf-8"
        )
    ...
    def test_migration_redefines_both_auth_level_functions(self) -> None:
        self.assertIn("CREATE OR REPLACE FUNCTION fn_map_org_auth_level(", self.migration_sql)
        self.assertIn("CREATE OR REPLACE FUNCTION fn_map_org_auth_level_rule(", self.migration_sql)
        self.assertIn("normalized_org_unit_name = '万睿科技'", self.migration_sql)
